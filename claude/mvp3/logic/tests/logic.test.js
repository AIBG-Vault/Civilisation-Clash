...
describe('Action validation', () => {
  ...
  describe('MOVE validation', () => {
    it('validates valid move', () => {
      ...
      const result = validateAction(state, 0, {
        action: ACTIONS.MOVE, from_x: 2, from_y: 5, to_x: 3, to_y: 5
      });
    ...
    it('rejects move from empty tile', () => {
      const result = validateAction(state, 0, {
        action: ACTIONS.MOVE, from_x: 3, from_y: 3, to_x: 4, to_y: 3
      });
    ...
    it('rejects move of enemy unit', () => {
      state.units.push({
        x: 2, y: 5, owner: 1, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
      });
    ...
    it('rejects move to impassable terrain', () => {
      state.units.push({
        x: 1, y: 1, owner: 0, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
      });

      const result = validateAction(state, 0, {
        action: ACTIONS.MOVE, from_x: 1, from_y: 1, to_x: 0, to_y: 1
      });
    ...
    it('rejects move exceeding movement distance', () => {
      ...
      const result = validateAction(state, 0, {
        action: ACTIONS.MOVE, from_x: 2, from_y: 5, to_x: 4, to_y: 5
      });
    ...
    it('allows raider to move 2 tiles', () => {
      state.units.push({
        x: 2, y: 5, owner: 0, type: UNIT_TYPES.RAIDER, hp: 1, canMove: true
      });
  ...
  describe('BUILD_UNIT validation', () => {
    ...
    it('rejects build at non-existent city', () => {
      const result = validateAction(state, 0, {
        action: ACTIONS.BUILD_UNIT, city_x: 5, city_y: 5, unit_type: UNIT_TYPES.SOLDIER
      });
    ...
    it('rejects build at enemy city', () => {
      const result = validateAction(state, 0, {
        action: ACTIONS.BUILD_UNIT, city_x: 8, city_y: 5, unit_type: UNIT_TYPES.SOLDIER
      });
    ...
    it('rejects build when city blocked', () => {
      state.units.push({
        x: 1, y: 5, owner: 0, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
      });
  ...
  describe('EXPAND_TERRITORY validation', () => {
    it('validates valid expansion', () => {
      const result = validateAction(state, 0, {
        action: ACTIONS.EXPAND_TERRITORY, x: 3, y: 5
      });
    ...
    it('rejects expansion to already owned tile', () => {
      const result = validateAction(state, 0, {
        action: ACTIONS.EXPAND_TERRITORY, x: 2, y: 5
      });
    ...
    it('rejects expansion not adjacent to territory', () => {
      const result = validateAction(state, 0, {
        action: ACTIONS.EXPAND_TERRITORY, x: 5, y: 3
      });
...
describe('Zone of Control', () => {
  ...
  it('soldier traps raider within 2 tiles', () => {
    ...
    state.units.push({
      x: 5, y: 3, owner: 1, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
    });

    // Friendly raider at (5, 5) - within 2 tiles
    const raider = {
      x: 5, y: 5, owner: 0, type: UNIT_TYPES.RAIDER, hp: 1, canMove: true
    };
  ...
  it('soldier does not trap other soldiers', () => {
    ...
    const soldier = {
      x: 5, y: 5, owner: 0, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
    };
  ...
  it('raider outside ZoC range can move', () => {
    ...
    const raider = {
      x: 5, y: 6, owner: 0, type: UNIT_TYPES.RAIDER, hp: 1, canMove: true
    };
    state.units.push(raider);
...
describe('Turn processing', () => {
  ...
  describe('Movement phase', () => {
    ...
    it('raiding enemy territory makes it neutral', () => {
      state.units.push({
        x: 6, y: 5, owner: 0, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
      });
  ...
  describe('Combat phase', () => {
    ...
    it('killing unit awards kill bonus', () => {
      ...
      const combatEvent = info.turnEvents.find(
        e => e.type === 'COMBAT' && e.data.isKill
      );
      assert.ok(combatEvent);
  ...
  describe('Build phase', () => {
    it('builds unit at city', () => {
      const { newState } = processTurn(state, {
        player0: [{ action: ACTIONS.BUILD_UNIT, city_x: 1, city_y: 5, unit_type: UNIT_TYPES.SOLDIER }],
        player1: []
      });
  ...
  describe('Scoring phase', () => {
    it('awards monument score to controller', () => {
      state.units.push({
        x: 5, y: 4, owner: 0, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
      });
  ...
  describe('City capture', () => {
    it('soldier captures enemy city instantly', () => {
      state.units.push({
        x: 7, y: 5, owner: 0, type: UNIT_TYPES.SOLDIER, hp: 3, canMove: true
      });
