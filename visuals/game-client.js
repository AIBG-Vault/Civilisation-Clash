...
class GameClient {
  constructor() {
    ...
    this.MIN_RENDER_INTERVAL = 50; // Minimum 50ms between renders (20 FPS)
  ...
  handleMessage(message) {
    switch (message.type) {
      case 'AUTH_SUCCESS':
        ...
        document.getElementById('status').textContent = `Playing as ${
  ...
  handleTileClick(x, y) {
    ...
    if (this.expandMode) {
      if (tile.type === 'FIELD' && tile.owner === null) {
        ...
        const hasOurTerritory = adjacent.some((t) => t.owner === this.myTeamId);
  ...
  throttledRender() {
    ...
    if (timeSinceLastRender >= this.MIN_RENDER_INTERVAL) {
      ...
      if (!this.renderThrottleTimer) {
        const timeToWait = this.MIN_RENDER_INTERVAL - timeSinceLastRender;
        this.renderThrottleTimer = setTimeout(() => {
          if (this.pendingRender && this.gameState) {
            this.renderer.render(this.gameState);
  ...
  toggleTimeout() {
    if (!this.isAdmin || !this.ws || this.ws.readyState !== WebSocket.OPEN) return;

    this.timeoutEnabled = !this.timeoutEnabled;
    const action = this.timeoutEnabled ? 'ENABLE_TIMEOUT' : 'DISABLE_TIMEOUT';

    this.ws.send(
      JSON.stringify({
        type: 'ADMIN_COMMAND',
        action: action,
        timeout: 30000, // Keep 30 second timeout when enabled
      })
    );
    ...
    if (this.timeoutEnabled) {
      ...
      btn.textContent = 'Enable Timeout';
      btn.style.background = '#4CAF50';
  ...
  toggleThrottle() {
    ...
    if (this.throttleEnabled) {
      ...
      btn.style.background = '#f44';
      console.log('Render throttling enabled (20 FPS max)');
    } else {
